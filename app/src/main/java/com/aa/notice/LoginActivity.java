...
public void submitLogin(int submit_type,String member_id, String pwd,String authorization_code) {
    ...
    OkHttpManager.getInstance().post(params, submit_url, new OkHttpManager.HttpCallBack() {
        @Override
        public void onResponse(JSONObject jsonObject0) {
            //解析json。数据存起来到sp
            try {
                dismissProgress();
//                        {"code":1,"msg":"登录成功","data":{"id":1,"name":"admin","phone":"15626277799","version":"1.1.1","money":"8614.170"}}
//                        {"code":0,"msg":"用户不存在"}
                System.out.println(">>>>>>>>>>>>登陆返回" + jsonObject0.toString());
                if (isJson(jsonObject0.toString())) {
                    //解析type
                    org.json.JSONObject jsonObj = new org.json.JSONObject(jsonObject0.toString());
                    String code = jsonObj.optString("code", "");
                    ...
                    if (code.equals("1")) {
                        ...
                        SPUtils.getInstance().put("nongxin_type", jsonObject.optInt("ysftype", -1));//类型
                        showToast("登录成功！！！");
                        startIntent(MainActivity.class);
                        finish();
                    } else {
